...
- **Not every `@immutable` domain type defines `==`/`hashCode`.** Only the types something actually compares by value do (`Quantity`, the `ProductionWarning` family); leaving the rest at identity equality is a scope decision for types no caller needs to compare, not an oversight.
